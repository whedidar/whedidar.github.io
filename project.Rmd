...
### Section 1: Analysis of tweeting times  &nbsp;
...
combined %>%
  ...
  labs(title = "Fig1. Percentage of tweets based on time of day", 
       x = "Hour of day (EST)", 
       y = "% of tweets", color = "",
       fill = "User") + 
...
### Section 2: Tweets with pictures/links &nbsp;
...
ggplot(pic_ct, aes(x = user, y = n, fill = picture)) +
  ...
  labs(title = "Fig2. Tweets with(out) picture/link", x = "", y = "Number of tweets", fill = "") +
  theme_bw()

```


### Section 3: Frequency of individual words tweeted &nbsp;
...
tweet_txt %>%
  ...
  ylab("Fig3. Occurrences") + xlab("Word") +
  coord_flip() + theme_bw()
...
### Section 5: Predictive algorithm &nbsp;



For our predictive algorithm, we select three input variables of interest:

1. whether the tweet has a picture/link, 
2. the time of the tweet, and 
3. a sentiment score using the AFINN dictionary. 
...
### Applying the algorithm to unrelated data  &nbsp;
...
We also observe from the confusion matrix below that while the algorithm still somewhat accurately categorizes tweets for either Jojo or Joe and does not break, we do note that the accuracy of predictive power decreases by about 7% compared to the original algorithm built using Elon and Tim's tweets. 
...
According to the decision tree, only the variables of a) pictures/link and b) tweet sentiment are predictive in this model; the time of the tweet is not a significant predictor of user, since it is not shown in the tree. 
